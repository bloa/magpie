    ...
    def get_contents(cls, file_path):
        with open(file_path) as config_file:
            for line in config_file.readlines():
                ...
                m = re.match("^CLI_PREFIX\s*=\s*\"([^\"]*)\"", line)
                if m:
                    cls.CLI_PREFIX = m.group(1)
                    continue
                m = re.match("^CLI_GLUE\s*=\s*\"([^\"]*)\"", line)
                if m:
                    cls.CLI_GLUE = m.group(1)
                    continue
                m = re.match("^CLI_BOOLEAN\s*=\s*\"([^\"]*)\"", line)
                if m:
                    cls.CLI_BOOLEAN = m.group(1)
                    continue
                m = re.match("^CLI_BOOLEAN_PREFIX_TRUE\s*=\s*\"([^\"]*)\"", line)
                if m:
                    cls.CLI_BOOLEAN_PREFIX_TRUE = m.group(1)
                    continue
                m = re.match("^CLI_BOOLEAN_PREFIX_FALSE\s*=\s*\"([^\"]*)\"", line)
                if m:
